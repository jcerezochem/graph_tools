program do_vib_analysis
    ...
    do i=1,iargc()
        ...
        select case (adjustl(arg))
            ...
            case ("-h")
                print*, "Usage: "
                print*, " do_spectral_analysis [-units (eV/cm)] < spectrum.dat"
                print*, ""
                stop
...
!    RM2=RM2/2.d0 * df
    ...
    factor=1.23981d-4
    print*, "DATA IN eV"
    print*, " M1         sigma      wmax      FWHM(grace)  FWHM(alter)   FWHM-from-sigma"
    print*, "-----------------------------------------------------------------------------"
    print'(5(F8.4,3X),5X,F8.4)', RM1*factor, sigma*factor, freqmax*factor, fwhm_gr*factor, fwhm*factor, &
                          2.d0*sigma*dsqrt(2.d0*log(2.d0))*factor
    print*, ""

    print*, "DATA IN cm-1"
    print*, " M1         sigma      wmax      FWHM(grace)  FWHM(alter)   FWHM-from-sigma"
    print*, "-----------------------------------------------------------------------------"
    print'(5(F8.1,3X),5X,F8.1)', RM1, sigma, freqmax, fwhm_gr, fwhm, 2.d0*sigma*dsqrt(2.d0*log(2.d0))
